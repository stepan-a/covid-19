...
def chart(data, data_rolling, countries, by_million_inh = False, align_curves = False, last_d = 15, offset_name = 'offset_confirmed', type_ppl = "confirmed cases", name_fig="", since=False, min_rate=0):
    ### Symbols
    symbols = []
    for i in range(35):
    ...
    for c in countries:
        ...
        """
        k = -1
        while np.isnan(y.iloc[k]) and abs(k) < len(x):
            k -=1
        y = y.iloc[k]

        #if True in (y_an - ay) > max(y_an)/20:
        ...
        """
        i+=1
        if i >= min(len(colors), len(symbols)):
            i=0
    ...
    fig.update_layout(
        ...
        xaxis_title="Time (day) {}".format(delayed),
        #yaxis_type="log",
    ...
    if export:
        fig.write_image("images/charts/{}.png".format(name_fig), scale=3, width=1100, height=700)
